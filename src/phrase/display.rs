...
impl<'w> DiagramNode<'w> {
    ...
    pub fn highlight(&self, stream :&mut dyn std::io::Write) {
        write!(stream, "<div id=highlightbox>{}</div>", self.highlight_(0)).unwrap();
    }
    pub fn highlight_css(stream :&mut dyn std::io::Write) {
        write!(stream, "<style>
        #highlightbox .d{{display:inline;background-image:url(https://media.discordapp.net/attachments/849082780863168532/862924869184651294/triangle-1.png?width=720&height=625);background-size:100% 100%;}}
